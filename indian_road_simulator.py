...
""")
    ...
    # Generate FLOWS for continuous traffic throughout the 2-hour simulation
    # Flows will continuously spawn vehicles until end time (7200 seconds)
    flow_id = 0
    vehicle_types = [
        ("auto", 20),       # 20 vehicles per hour
        ("motorbike", 30),  # 30 vehicles per hour (more frequent)
        ("car", 25),        # 25 vehicles per hour
        ("bus", 15)         # 15 buses per hour (less frequent)
    ]
    
    for vtype, veh_per_hour in vehicle_types:
        # Calculate period: 3600 seconds / vehicles_per_hour
        period = 3600.0 / veh_per_hour
        
        # Create multiple flows from different start points for variety
        for flow_num in range(5):  # 5 flows per vehicle type
            ...
            # Stagger flow start times to avoid all spawning at once
            begin_time = flow_num * (period / 5)
            
            f.write(f'    <flow id="{vtype}_flow_{flow_id}" type="{vtype}" begin="{begin_time:.1f}" end="7200" period="{period:.1f}" from="{from_edge}" to="{to_edge}" departLane="best" departSpeed="max"/>\n')
            flow_id += 1
    
    f.write('</routes>\n')

print(f"Created {flow_id} continuous vehicle flows (total ~90 vehicles/hour for 2 hours)")
